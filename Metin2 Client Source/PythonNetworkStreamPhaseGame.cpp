...
#ifdef ENABLE_CHESS_SYSTEM
bool CPythonNetworkStream::RecvChessPacket()
{
	TPacketGCChess pack;
	...
	{
	case CHESS_SUBHEADER_GC_INVITE:
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BinaryServerCommand", Py_BuildValue("(s)", ("chess_invite " + std::string(pack.szName)).c_str()));
		break;
	case CHESS_SUBHEADER_GC_START:
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BinaryServerCommand", Py_BuildValue("(s)", ("chess_start " + std::to_string(pack.arg1) + " " + std::to_string(pack.arg2) + " " + std::string(pack.szName)).c_str()));
		break;
	case CHESS_SUBHEADER_GC_MOVE:
		{
			int from_x = pack.arg1 >> 8;
			int from_y = pack.arg1 & 0xFF;
			int to_x = pack.arg2 >> 8;
			int to_y = pack.arg2 & 0xFF;
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BinaryServerCommand", Py_BuildValue("(s)", ("chess_move " + std::to_string(from_x) + " " + std::to_string(from_y) + " " + std::to_string(to_x) + " " + std::to_string(to_y)).c_str()));
		}
		break;
	case CHESS_SUBHEADER_GC_UPDATE:
		{
			int x = pack.arg1 >> 8;
			int y = pack.arg1 & 0xFF;
			int piece = pack.arg2;
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BinaryServerCommand", Py_BuildValue("(s)", ("chess_update " + std::to_string(x) + " " + std::to_string(y) + " " + std::to_string(piece)).c_str()));
		}
		break;
	case CHESS_SUBHEADER_GC_END:
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BinaryServerCommand", Py_BuildValue("(s)", "chess_end"));
		break;
	}
...
bool CPythonNetworkStream::SendChessPacket(BYTE subheader, const char* szName, int arg1, int arg2)
{
	TPacketCGChess pack;
	pack.header = HEADER_CG_CHESS;
	pack.subheader = subheader;
	pack.arg1 = arg1;
	pack.arg2 = arg2;
	strncpy(pack.szName, szName, sizeof(pack.szName) - 1);
	pack.szName[sizeof(pack.szName) - 1] = '\0';
