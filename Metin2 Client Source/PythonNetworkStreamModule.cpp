...
PyObject* netSendChessPacket(PyObject* poSelf, PyObject* poArgs)
{
	int iSubHeader;
	...
	char* szName = "";
	PyTuple_GetString(poArgs, 1, &szName);

	int iArg1 = 0;
	PyTuple_GetInteger(poArgs, 2, &iArg1);

	int iArg2 = 0;
	PyTuple_GetInteger(poArgs, 3, &iArg2);

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendChessPacket(iSubHeader, szName, iArg1, iArg2);
...
#ifdef ENABLE_CHESS_SYSTEM
		{ "SendChessPacket",					netSendChessPacket,						METH_VARARGS },
#endif
