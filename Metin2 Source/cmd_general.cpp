...
ACMD(do_chess)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Usage: /chess <invite|accept|decline|quit|bot>");
		return;
	}
	...
	if (!strcmp(arg1, "invite"))
	{
		if (!*arg2)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Usage: /chess invite <name>");
			return;
		}
		CChessManager::instance().Invite(ch, arg2);
	}
	else if (!strcmp(arg1, "accept"))
	{
		if (!*arg2)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Usage: /chess accept <name>");
			return;
		}
		CChessManager::instance().Accept(ch, arg2);
	}
	else if (!strcmp(arg1, "decline"))
	{
		if (!*arg2)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Usage: /chess decline <name>");
			return;
		}
		CChessManager::instance().Decline(ch, arg2);
	}
	else if (!strcmp(arg1, "quit"))
	{
		CChessManager::instance().Quit(ch);
	}
	else if (!strcmp(arg1, "bot"))
	{
