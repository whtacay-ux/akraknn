...
#ifdef ENABLE_CHESS_SYSTEM
void CInputMain::Chess(LPCHARACTER ch, const char* data)
{
	if (!ch)
		return;
	...
	TPacketCGChess* p = (TPacketCGChess*)data;
	switch (p->subheader)
	{
	case CHESS_SUBHEADER_CG_INVITE:
		CChessManager::instance().Invite(ch, p->szName);
		break;
	case CHESS_SUBHEADER_CG_ACCEPT:
		CChessManager::instance().Accept(ch, p->szName);
		break;
	case CHESS_SUBHEADER_CG_DECLINE:
		CChessManager::instance().Decline(ch, p->szName);
		break;
	case CHESS_SUBHEADER_CG_MOVE:
		CChessManager::instance().Move(ch, p->arg1 >> 8, p->arg1 & 0xFF, p->arg2 >> 8, p->arg2 & 0xFF);
		break;
	case CHESS_SUBHEADER_CG_QUIT:
		CChessManager::instance().Quit(ch);
		break;
	case CHESS_SUBHEADER_CG_START_BOT:
		CChessManager::instance().StartBot(ch);
		break;
	}
}
#endif
